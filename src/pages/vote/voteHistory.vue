<template>
  <div class="main-content">
    <div class="center vote-history">
      <BackBar :backTitle="$t('message.voteBackName')" backUrl="/voteList"></BackBar>
      <h3 class="h3">
        <span>{{$t('message.voteList8')}}</span>
      </h3>
      <div class="vote-list-content">
        <div class="search flex">
          <div class="search-left flex">
            <div class="create-vote-btn" @click="createVote">
              <el-button type="primary">{{$t('message.voteList6')}}</el-button>
            </div>
            ...
            <div class="search-box">
              ...
              <input v-model="keyword" class="search-ipt" type="text" :placeholder="$t('message.voteList15')" @change="search">
          ...
          <tr v-for="item in txList" :class="item.status===4 ? 'gray':''">
            <td :data-label="$t('message.voteList9')" class="overflow cursor-p"  @click="voteDetail(item.id)">
              <el-tooltip class="item" effect="dark" :content="item.voteCanModify ? $t('message.createVote25'):''" placement="bottom-start">
                <i class="modify iconfont iconfont-common-green cursor-p" :class="item.status===4 ? 'iconfont-common-gray' : 'iconfont-common-green'" v-show="item.voteCanModify">&#xe627;</i>
              </el-tooltip>
              <span :class="item.voteCanModify ? '': 'title-text'">{{item.title}}</span></td>
...
<script>
  ...
  import {getVotesHistoryList} from '@/api/httpData.js'
  export default {
    data() {
      return {
        //address
        ...
        totalAllOk:false,
        keyword: '',
        voteHistorySetInterval:null,
        voteHistorySetInterval2:null
      }
    },
    computed: {
      addressAlias () {
    ...
    mounted() {
      this.voteHistorySetInterval = setInterval(() => {
        this.getVoteList();
      }, 10000);
    },
    activated(){
    ...
    methods: {
     ...
     //vote list
      ...
      voteDetail(id){
        this.$router.push({
          name: '/voteDetail',
          query: {voteId: id,routerName:'voteHistory'}
        })
      },
    },
